"""Add tuning result explanation column to NEXT_SQL_INFO.
...
def main() -> None:
    ...
    with get_connection() as conn:
        cur = conn.cursor()
        if column_exists(cur, short_table_name, "TUNED_RESULT"):
            print("TUNED_RESULT already exists.")
        else:
            cur.execute(f"ALTER TABLE {table_name} ADD (TUNED_RESULT CLOB)")
            print("Added TUNED_RESULT.")
        conn.commit()
    print("Done.")
